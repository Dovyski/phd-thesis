...
development company, where he worked for years in a variety of applications and
...
\\
\\
In his thesis, Fernando presents a method for remote detection of emotions
(boredom and stress) of a person during the interaction with games. Player's
heart rate and facial actions are used in the process, which are automatically
and remotely calculated from a video feed using Computer Vision. Since the
information is acquired remotely, specialized equipment like physical sensors
is not required, only an ordinary camera and a computer are needed. Fernando
supports the creation and testing of the proposed method with a series of
systematic evaluations conducted to better understand the relation between
psychophysiological signals and player's emotions. His thesis is an initiative
to move away from questionnaires and physical sensors and into a non-obtrusive,
user-tailored, and remote-based approach for detecting user emotions.
}{%
