...
Facial expressions are part of the human communication. Schemes such as the Facial Action Coding System \cite{ekman1977facial,cohn2007observer} aim to standardize the measurements of facial expression by defining highly regulated procedural techniques to detect facial actions/movements. When analyzed in a defined context, such mapped actions can present a correlation between determined facial features and emotional states, e.g. stress and boredom. The analysis of facial expressions is being used in a variety of fields to help the understanding of individuals or the enhancement of their experience, which is the case of (serious) games.
...
In such a context, if any emotional state can be extracted from the analysis of the player's face, the game itself could use such information to adjust the current difficulty level, ensuring a challenging and fun experience. In particular if any information regarding the current stress and boredom levels of the player can be obtained in a non-intrusive way, a game designer could leverage that information to create more dynamic and customized games. Those games would be tailored to the needs of each individual, which results in a unique experience that prevents unnecessary frustration, stress and boredom for the player. In the case of serious games, a self-balancing game could be more engaging, which is proven to increase the player's attention to the current learning task \cite{cowie2001emotion}.
