...
In such a context, if any emotional state can be extracted from the analysis of the player's face, the game itself could use such information to adjust the current difficulty level, ensuring a challenging and fun experience. In particular if any information regarding the current stress and boredom levels of the player can be obtained in a non-intrusive way, a game designer could leverage that information to create more dynamic and customized games. Those games would be tailored to the needs of each individual, which results in a unique experience that prevents unnecessary frustration, stress and boredom for the player. %In the case of serious games, a self-balancing game could be more engaging, which is proven to increase the player's attention to the current learning task \parencite{cowie2001emotion}.
