...
Features and their classification potential are highly dependent on the type of data being used. In the present study, the set of features used for classification was extracted and selected based on previous reports of said features to differentiate emotional states in games. In total 8 features, denoted $F_1$ to $F_8$, are used: $F_1$ to $F_7$ are related to facial activity and $F_8$ is related to HR activity, estimated remotely. Table \ref{table:study5-features-list} presents a description of all features.
...
In order to test the effectiveness of the neural network in classifying samples as either boredom or stress, all trained neural networks were evaluated in conjunction. As described in the previous section, each subject's model was evaluated using LOSOCV, which produced a classification accuracy $A_i$ for any given subject $S_i$. The minimum, maximum and mean value of $A_i$ was calculated as a metric for accuracy. The same process was also applied to other three metrics obtained during the LOSOCV evaluation: precision, recall and F1 score.
...
\begin{table*}
    ...
    \begin{tabular}[l]{@{}lp{9cm}}
        \hline
            \textbf{Test ($T_i$)} & \textbf{Feature set} \\
        \hline
            1 & Mouth outer Monitor the zygomatic muscle.  \\
            2 & Mouth corner Monitor the zygomatic muscle. \\
            $T_3$ & Eye area Monitor the orbicularis oculi muscle. \\
            $T_4$ & Eyebrow activity Monitor the corrugator muscle.  \\
            $T_5$ & Face area Monitor facial movement to and away from the camera  \\
            $T_6$ & Face motion Describe the total distance the head has moved in any direction in a short period of time.  \\
            $T_7$ & Facial COM Describe the overall movement of all facial landmarks. \\
            $T_8$ & Remote HR HR estimated using the rPPG technique proposed by \textcite{poh2011advancements}.  \\
        \hline
    \end{tabular}
\end{table*}
...
Subjects perceived the games as boring at the beginning and stressful at the end, so it is expected that values in $H_0$ and $H_1$, for all features, are likely to correlate with an emotional state of boredom and stress, respectively. Such difference should make a trained neural network capable of properly classifying evaluation samples as either boredom or stress. Additionally the use of a multifactorial approach, where facial analysis and HR information are used in combination instead of either one alone, is expected to produce better classification results \parencite{zacharatos2014automatic}. Based on those expectations, the following hypotheses are stated:

\begin{itemize}
  \item[] $u_1$: a user-tailored neural network trained on data samples from two calibration games of a given subject $S_i$ is able to classify samples from a third calibration game of that same subject $S_i$ with an accuracy greater than 50\% (random guessing);
  \item[] $u_2$: a user-tailored neural network using a multifactorial feature set, i.e. facial and HR features, performs with greater accuracy than an user-tailored neural network using facial features only;
  \item[] $u_3$: a user-tailored neural network using a multifactorial feature set, i.e. facial and HR features, performs with greater accuracy than an user-tailored neural network using HR features only;
\end{itemize}

Hypothesis $u_1$ was tested by checking if the mean value of the classification accuracy, i.e. calculated from all $A_i$ values, is greater than 0.5. Hypotheses $u_2$ and $u_3$ were tested by performing a Wilcoxon signed-rank test on the $A_i$ values of the two competing models.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Results}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Table \ref{table:study5-tests-metrics} presents the mean of differences of all features between periods $H_0$ and $H_1$, calculated for all subjects in all games and analyzed according to the procedures described in Section \ref{s:experiment1-study5-method}.

\begin{table*}
    \centering
    \caption{Mean value of resulting classification metrics}
    \label{table:study5-result-metrics-mean}
    \begin{tabular}[l]{@{}ccccc}
        \hline
            \textbf{Test} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1}\\
        \hline
            1 & 0.31 & 0.92 & 0.6143 & 0.6143 \\
            2 & 0.31 & 0.92 & 0.6143 & 0.6143 \\
            3 & 0.31 & 0.92 & 0.6143 & 0.6143 \\
        \hline
    \end{tabular}
\end{table*}
...
\begin{table}[!htbp]
\centering
\caption{Minumum and maximum values of resulting classification metrics}
...
\hline
1  & 31 & 92 & 61.43 & 32 & 90 & 62.50 & 32 & 90 \\
3  & 31 & 92 & 61.43 & 32 & 90 & 62.50 & 32 & 90 \\
\hline
\end{tabular}
\end{table}
