...
\label{s:study5-training-evaluation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In order to test the effectiveness of the neural network in classifying evaluation samples as either boredom or stress, each trained neural network was evaluated on a subject level. As described in the previous section, each subject's model was evaluated using LOSOCV, which produced a classification accuracy $A_i$ for any given subject $S_i$. The minimum, maximum and mean value of $A_i$ was calculated as a metric for accuracy. The same process was also applied to other three metrics obtained during the LOSOCV evaluation: precision, recall and F1 score.

Aiming to better understand the contribution of each feature for the classification process, the training/evaluation process mentioned early was also performed using different feature sets. Each of those different feature set was evaluated in an independent tested, denoted $T_i$. Table \ref{table:study5-different-feature-sets} shows the different feature sets used on each test $T_i$

\begin{table*}
    \centering
    \caption{Tests and their respective feature sets}
    \label{table:study5-different-feature-sets}
    \begin{tabular}[l]{@{}lp{6.5cm}}
        \hline
            \textbf{Test} & \textbf{Feature set} \\
        \hline
            $T_1$ & Mouth outer Monitor the zygomatic muscle.  \\
            $T_2$ & Mouth corner Monitor the zygomatic muscle. \\
...
Subjects perceived the games as boring at the beginning and stressful at the end, so it is expected that values in $H_0$ and $H_1$, for all features, are likely to correlate with an emotional state of boredom and stress, respectively. Such difference should make a trained neural network capable of properly classifying evaluation samples as either boredom or stress. Additionally the use of a multifactorial approach, where facial analysis and HR information are used in combination instead of either one alone, is expected to produce better classification results \parencite{zacharatos2014automatic}.

Consequentially, the following hypotheses are stated:

\begin{itemize}
  \item[] \textbf{$u_1$} features are likely to correlate with an emotional state of boredom and stress, respectively.
  \item[] \textbf{$u_2$} features are likely to correlate with an emotional state of boredom and stress, respectively.
  \item[] \textbf{$u_3$} features are likely to correlate with an emotional state of boredom and stress, respectively.
\end{itemize}
